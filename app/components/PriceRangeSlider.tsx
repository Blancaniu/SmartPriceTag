    ...
    <div>
      <div className="mb-3 flex justify-between gap-4">
        {(["min", "max"] as const).map(endpoint => (
          <div key={endpoint} className={endpoint === "max" ? "text-right" : ""}>
            <span className="mb-1 block text-xs text-[#536B50]">{endpoint === "min" ? "Minimum price" : "Maximum price"}</span>
            {editing === endpoint ? (
              <input autoFocus type="number" step="0.01" min={endpoint === "min" ? 0 : value.min}
                ...
                className="w-28 rounded-lg border border-[#2D7545] bg-white px-2 py-1 text-sm font-bold outline-none" />
            ) : (
              <button type="button" aria-label={`Edit ${endpoint === "min" ? "minimum" : "maximum"} price`}
                ...
                className="rounded-lg border border-slate-200 px-3 py-1 text-sm font-bold hover:border-[#2D7545]">
      ...
      <div className="price-range-slider relative h-8">
        <div className="absolute inset-x-0 top-3 h-2 rounded-full bg-slate-100">
          <div className="absolute h-full rounded-full bg-[#2D7545]" style={{ left: `${lowerPercent}%`, width: `${upperPercent - lowerPercent}%` }} />
        </div>
      ...
      <p className="mt-1 text-xs text-[#536B50]">Drag either handle or click a price to enter a number.</p>
