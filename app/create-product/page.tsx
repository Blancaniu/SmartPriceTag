...
function CreateProductPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F5] text-[#203B2A] py-8">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        {/* Navigation & Header */}
        <div className="mb-6 flex items-center justify-between">
          <Link
            href="/business"
            className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-[#203B2A] transition-all hover:border-[#2D7545]"
          >
            <ArrowLeft className="h-4 w-4 text-[#2D7545]" />
            Back to Dashboard
          </Link>

          <span className="text-xs font-bold text-[#536B50]">
            Retail Inventory Upload Form
          </span>
        </div>

        <div className="mb-8 border-b border-slate-200/80 pb-4">
          <h1 className="text-3xl font-black text-[#203B2A] flex items-center gap-2">
            <PlusCircle className="h-8 w-8 text-[#2D7545]" />
            Upload New Food Product
          </h1>
          <p className="text-xs text-[#536B50] mt-1">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7">
            <form onSubmit={handleSubmit} className="space-y-5 bg-white p-6 rounded-3xl border border-slate-200/90">
              ...
              {successMsg && (
                <div className="rounded-xl bg-[#2D7545]/10 p-3 text-xs font-bold text-[#203B2A] border border-[#2D7545]/30">
                  <CheckCircle2 className="inline h-4 w-4 mr-1 text-[#2D7545]" />
                  {successMsg}
                </div>
              )}
              ...
              <div>
                ...
                <input
                  ...
                  className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-semibold text-[#203B2A] outline-none focus:border-[#2D7545] focus:bg-white"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-bold text-[#203B2A] outline-none focus:border-[#2D7545] focus:bg-white"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs font-semibold text-[#203B2A] outline-none focus:border-[#2D7545] focus:bg-white"
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full py-3 rounded-xl border border-[#203B2A] bg-[#203B2A] text-xs font-bold text-white hover:bg-[#2D7545] transition-colors"
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="rounded-3xl border border-[#2D7545]/30 bg-gradient-to-br from-[#FAF9F5] via-white to-[#2D7545]/10 p-6">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-4">
                <span className="text-xs font-extrabold uppercase tracking-wider text-[#203B2A] flex items-center gap-1.5">
                  <Sparkles className="h-4 w-4 text-[#2D7545]" />
                  Live AI Pricing Calculation
                </span>
                <span className="rounded-full bg-[#2D7545]/10 border border-[#2D7545]/30 px-2.5 py-0.5 text-[10px] font-bold text-[#203B2A]">
                  PREVIEW
                </span>
              </div>
              ...
              <h3 className="text-xl font-bold text-[#203B2A] truncate mb-1">
                {name || "Product Name"}
              </h3>
              <p className="text-xs text-[#536B50] mb-4">{brand || "Brand"} &bull; {category}</p>
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-4 mb-4">
                <div className="flex items-baseline justify-between">
                  <div>
                    ...
                    <span className="text-3xl font-black text-[#203B2A]">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-bold">
                  <span className="text-[#536B50]">Freshness Score:</span>
                  <span className="text-[#203B2A]">{aiPricingPreview.freshnessScore}% ({aiPricingPreview.label})</span>
                </div>
                <div className="h-2.5 w-full rounded-full bg-slate-100 border border-slate-200 overflow-hidden">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-red-500 via-amber-400 to-[#2D7545]"
